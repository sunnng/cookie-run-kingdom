...
--- 临时覆盖引擎参数（识别后需 applyEngine 还原）
local function applyConfig(cfg)
	if type(cfg) ~= "table" then
		return
	end
	if cfg.recType then
		engine.setRecType(cfg.recType)
	end
	if cfg.detBoxType then
		engine.setDetBoxType(cfg.detBoxType)
	end
	if cfg.detScaleRatio then
		engine.setDetScaleRatio(cfg.detScaleRatio)
	end
	if cfg.detUnclipRatio then
		engine.setDetUnclipRatio(cfg.detUnclipRatio)
	end
	if cfg.recScoreThreshold then
		engine.setRecScoreThreshold(cfg.recScoreThreshold)
	end
	if cfg.runMode then
		engine.setRunMode(cfg.runMode)
	end
	if cfg.filterColor ~= nil or cfg.filterBg then
		engine.setFilterColor(cfg.filterColor or ENGINE.FILTER_COLOR, cfg.filterBg or ENGINE.FILTER_MODE)
	end
	if cfg.binaryThresh then
		engine.setBinaryThresh(cfg.binaryThresh)
	elseif cfg.binaryThresh == 0 then
		engine.setBinaryThresh(0)
	end
end
...
--- @param cfg table|nil 临时 OCR 参数（兼容旧脚本 recognizeXxx）
function Ocr.scan(rect, mode, returnType, cfg)
	if not engine then
	...
	if useNumberRec and not (type(cfg) == "table" and cfg.recType) then
...
--- 识别数字
function Ocr.number(rect, cfg)
	local r = Ocr.scan(rect, ENGINE.LINE_MODE, "num", cfg)
	return r and tonumber(r.text) or nil
end

--- 兼容旧脚本：多行文本
function Ocr.recognizeText(rect, cfg)
	local r = Ocr.scan(rect, ENGINE.MULTI_MODE, "text", cfg)
	return r and r.text or ""
end

--- 兼容旧脚本：数字文本
function Ocr.recognizeNumber(rect, cfg)
	cfg = cfg or {}
	cfg.recType = cfg.recType or "number"
	cfg.returnType = "text"
	local r = Ocr.scan(rect, ENGINE.MULTI_MODE, "text", cfg)
	return r and r.text or nil
end

--- 兼容旧脚本：提取 words 数组
function Ocr.recognizeWords(rect, cfg)
	local r = Ocr.scan(rect, ENGINE.MULTI_MODE, "json", cfg)
	if not r or not r.items then
		return {}
	end
	local words = {}
	for _, item in ipairs(r.items) do
		if item.words and item.words ~= "" then
			words[#words + 1] = item.words
		end
	end
	return words
end
...
--- 轮询等待文字出现并点击其坐标
--- @param text string 目标文字（子串匹配）
--- @param rect table {x1,y1,x2,y2}
--- @param timeoutMs number|nil 默认 10000
--- @param intervalMs number|nil 默认 config 轮询间隔
--- @param delayMs number|nil 点击后延迟毫秒
--- @return boolean ok 是否找到并点击
--- @return number|nil x 点击坐标
--- @return number|nil y 点击坐标
function Ocr.waitTap(text, rect, timeoutMs, intervalMs, delayMs)
	if not text or text == "" then
		return false
	end
	if not validRect(rect) then
		Logger.warn("[OCR] waitTap 无效 rect: " .. tostring(rect))
		return false
	end
	timeoutMs = timeoutMs or 10000
	intervalMs = intervalMs or TIMING.POLL_INTERVAL_MS
