...
function Runtime.run()
	...
	while true do
		...
		if not hasWork then
			...
			local waitRemain = math.max(farRemain , miningRemain)
			if waitRemain > 0 then
				local idleSec = math.max(1 , math.floor(idleMs / 1000))
				Logger.info(string.format(
				TAG .. " [idle] 矿山等待 剩余%ds（勘查%d 开采%d）本轮tick %ds" ,
				waitRemain , farRemain , miningRemain , idleSec
				))
				for _ = 1 , idleSec do
					...
					waitRemain = math.max(farRemain , miningRemain)
					if waitRemain <= 0 then
						Logger.info(TAG .. " [idle] 矿山等待已到期")
						break
					end
