...
local d = Dialog.new(featureDef, { tag = "[模块.页面]" })

-- 单弹窗
d:handle({ mode = "flow", action = "confirm", waitAppearMs = 5000, required = true })
d:toGuardHandler({ action = "confirm", waitGoneMs = 2000 })  -- Guard 偶发

-- 顺序未知，0~N 个弹窗（如竞技场两个 confirm）
Dialog.resolveUntilIdle({
  { dialog = dA, name = "A", priority = 10, opts = { action = "confirm", waitGoneMs = 1000 } },
  { dialog = dB, name = "B", priority = 10, opts = { action = "confirm", waitGoneMs = 1000 } },
}, { timeoutMs = 8000, minWaitMs = 500, settleMs = 800, maxHandled = 2, tag = TAG })

-- 先 A 必现，点 A 后分支出 B 或成功（如交易所购买）
Dialog.resolveAfterPrimary({
  primary = { dialog = confirmD, opts = { mode = "flow", action = "confirm", waitAppearMs = 5000, required = true } },
  watch = { { dialog = shortageD, opts = { action = "cancel" }, result = "shortage", after = function() ... end } },
  successWhen = function() return not confirmD:isVisible() end,
...
| 场景 | API |
|------|-----|
| Guard 偶发单弹窗 | `toGuardHandler` / `handle(ifVisible)` |
| 流程单弹窗 | `handle(flow)` |
| 顺序确定的 optional 链 | `handleChain` |
| 顺序未知 0~N 弹窗 | **`resolveUntilIdle`** |
| 主弹窗后分支 B 或成功 | **`resolveAfterPrimary`** |
...
| 两种弹窗顺序未知仍用 handleChain | 用 `Dialog.resolveUntilIdle` |
| 主弹窗后分支出次弹窗 | 用 `Dialog.resolveAfterPrimary` |
